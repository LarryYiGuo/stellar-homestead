...
/* 20 级开发体系:几何曲线 ×1.557 —— 宜居度 1 时 LV10 ≈ 17 小时(第一天打满前 10 级),
   LV20 = 5184000 pts ≈ 60 天(长线放置目标;资源产率模型的 T 锚定此值,曲线不变) */
const LEVELS = [
  { name: '未开发',     th: -1 },
  { name: '前哨站',     th: 0 },
  { name: '聚居点',     th: 1800 },
  ...
  { name: '行星首府',   th: 62090 },
  { name: '轨道文明',   th: 96660 },
  { name: '共生都市群', th: 150475 },
  { name: '行星议会',   th: 234250 },
  { name: '改造纪元',   th: 364660 },
  { name: '绿洲网络',   th: 567700 },
  { name: '生态走廊',   th: 883760 },
  { name: '行星花园',   th: 1375800 },
  { name: '盖娅雏形',   th: 2141800 },
  { name: '盖娅之境',   th: 3334300 },
  { name: '生态文明',   th: 5184000 },
];
const MAX_LEVEL = 20;
const DEV_TRADE_BOOST = 3.32;   // 通商中的居住星:开发速度额外 +332%(总 ×4.32)

/* 21 级里程碑 + 渐近上限:类地 capScale=1.0(萨拉萨)生态极限 ~15亿,
   乘资源协同(≤×1.6)与建筑加成后 ≈ 30亿 —— 对标地球生态承载,不再有 400亿 */
const POP_MILESTONES = [0, 100, 500, 2e3, 1.5e4, 5e4, 1.5e5, 4e5, 1e6, 2.5e6,
  6e6, 1.4e7, 3e7, 6.5e7, 1.3e8, 2.5e8, 4.2e8, 6.5e8, 9.5e8, 1.25e9, 1.45e9, 1.5e9];
...
// 母港影响力:常驻全额产出,停靠母港锚地额外 +50%(见 port.js infRatePerMin)
...
const GATE_COST = { metal:6e4, chem:1.8e4, he3:6e3 };   // ≈ LV6 列车 3 趟货运量
const GATE_TIME = 1200;                              // 建造工期(秒)
const GATE_MAXLV = 5;
const GATE_RANGE = 11;                               // 作用圈半径:收集范围 = 单跳输送上限(银河边缘必须接力)
...
/* 人口加速(投入制):选定资源与数量,确认后开始燃烧;越稀有加速越强,最高 +200%/小时 */
const BOOST_TIER_RATE = { basic: 0.5, mid: 1.0, rare: 2.0 };   // +50% / +100% / +200% 每小时
const BOOST_BURN_MIN  = 0.2;                                   // 燃烧速率下限(/秒)
const BOOST_BURN_POPK = 3e-7;                                  // 燃烧速率 = max(下限, 人口×系数)/秒
const BOOST_AMOUNTS = [100, 500, 2000, 10000];   // 单轮投入档位

/* 类地化改造:仅「标准带」星球可改造(严酷星球无法改造),巨量资源 + 后期门槛 */
const TERRAFORM_COST = { metal:2.5e5, chem:8e4, ice:1e5, deut:5e4 };   // ≈ LV10 货舱 2 趟有余(终局工程)
...
const CAR_TECH_TIME = { 2:120, 3:300, 4:750, 5:1900 };         // 研发工期(秒):2分→32分,走研发槽不占工程槽
...
const CAR_COSTS = {
  ...
  6: { metal:2e4,  ice:6e3 },
  7: { metal:6e4,  he3:1.8e4 },
  8: { metal:1.5e5, deut:4.5e4 },
};
...
const ENGINE_COSTS = {
  2:  { metal:1.2e3 },
  3:  { metal:8e3,   ice:2.5e3 },
  4:  { metal:2e4,   he3:6e3 },
  5:  { metal:5e4,   deut:1.5e4 },
  6:  { metal:1.2e5, he3:5e4 },
  7:  { metal:3e5,   ice:1.2e5 },
  8:  { metal:7.5e5, chem:2.5e5 },
  9:  { metal:1.8e6, deut:6e5 },
  10: { metal:4.5e6, he3:1.2e6, deut:6e5 },
};
...
const BUILDINGS = {
  ...
  dome:       { name:'穹顶居住群',   district:'habitation', time:600,
    cond:{role:'hab', lv:4}, fx:{cap:0.12},  desc:'本星人口上限 +12%' },
  medbay:     { name:'再生医疗中心', district:'habitation', time:900,
    cond:{role:'hab', lv:7}, fx:{cap:0.10},  desc:'本星人口上限 +10%' },
