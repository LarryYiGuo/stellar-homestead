...
const INF_DOCK_HAB   = 0.05; // 每座居住区划 0.05 影响力/分(常驻产出,停靠锚地 ×1.5)
const INF_DOCK_TRADE = 0.4;  // 每座商贸区划 0.4 影响力/分
const INF_LINE_PER_MIN = 0.25;// 每条运行中的贸易线 0.25 影响力/分

const STARPORT_TIME = 240;   // 星港工期 4 分钟(独立建筑,不占区划)
function starportCost(nth){  // 第 N 座星港成本 +20%/座(100%/120%/140%…)
  const base = { metal: 1500, chem: 400 };
  const out = {};
...
/* 影响力加速定价:超线性 —— 小工程便宜,大工程昂贵;定价对齐影响力收入(约十几分钟收入换十分钟工期) */
function infRushCost(sec){
  const m = Math.max(1, Math.ceil(sec / 60));
  return Math.ceil(2 * m + m * m / 120);
}
