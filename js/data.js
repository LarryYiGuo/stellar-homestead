...
/* ── 开发阶段:阈值单位 = 开发点(1 点 = 1 真实秒 × 宜居度系数)
   EVE 式先快后慢,每级耗时 ×6:6.7 时 → 1.7 天 → 10 天 → 60 天(宜居 1.0 纯挂机)
   自行通商的居住星 ×4.32 → 满级约 14 天;玩家注资/影响力/剧情再加速 ── */
const LEVELS = [
  { name: '未开发',   th: -1 },
  { name: '前哨站',   th: 0 },
  { name: '聚居点',   th: 24000 },
  { name: '殖民地',   th: 144000 },
  { name: '行星都市', th: 864000 },
  { name: '生态文明', th: 5184000 },
];
const MAX_LEVEL = 5;
const DEV_TRADE_BOOST = 3.32;   // 通商中的居住星:开发速度额外 +332%(总 ×4.32)

const POP_MILESTONES = [0, 100, 2e3, 1.5e4, 5e6, 5e8, 2e10];
const RES_MILESTONES = [0, 50, 2e3, 8e4, 3e6, 8e7];
const RES_RATE_MAX = 1200;
const RES_SCALE = 0.04;    // 全局产量缩放:对齐列车容量/造价的数量级(原曲线快约 25 倍)
...
const CAR_TYPES = {
  ...
  cargo:   { name:'运输车厢', en:'TRANSPORT',  cap:800, pax:1000,
    desc:'标准化货舱。基础装载 800/节(随引擎 ×2.5/级,随车厢等级 +30%/级)。可无缝改为载人模式(1000 人),改装后不运货。' },
  weapon:  { name:'战斗车厢', en:'COMBAT', pax:100,
    desc:'装甲化武器底座,可安装 双联机关炮 / 激光炮 / 导弹井 并独立升级。可改为载人模式(100 人),改装后武器离线。' },
  ...
  general: { name:'通用车厢', en:'MULTIROLE', pax:500, cap:200, rp:0.02,
    desc:'四合一舱段:1/4 货运(200)+ 载人 500 + 1/4 科研,并可集成一门机关炮(火力随车厢等级成长,不可换装)。' },
  cryo:    { name:'休眠舱单元', en:'CRYO BAY', pax:4000,
    desc:'深空休眠迁移舱,单节一次运送 4000 名移民(随车厢等级 +30%/级)。殖民拓荒的主力运力。' },
...
function carTime(to){ return Math.round(60 * Math.pow(3, to - 2)); }   // 1分 → 27分
...
function rpcoefTime(to){ return Math.round(90 * Math.pow(2.5, to - 1)); }
...
const AMMO_PER_GUN = 12;          // 每门武器弹药基数(军事区停靠补满)
const CREW_BASE = 3;              // 基础乘员组(每组维护 2 节车厢,过半数方可运行)
const LOCAL_T_MIN = 5, LOCAL_T_MAX = 30;   // 星系内停靠转移耗时(秒)

/* ── 星港经济:影响力收入 / 星港 / 贸易线 / 资源仓 ── */
const INF_DOCK_HAB   = 1;    // 停靠锚地:每座居住区划 1 影响力/分
const INF_DOCK_TRADE = 3;    // 停靠锚地:每座商贸区划 3 影响力/分
const INF_HOMEPORT_DIV = 3;  // 母港:列车不在时按 1/3 速率持续产出
const INF_LINE_PER_MIN = 0.5;// 每条运行中的贸易线 0.5 影响力/分(每 2 分钟 1 点)

const STARPORT_TIME = 600;   // 星港工期 10 分钟(独立建筑,不占区划)
...
/* 星球资源仓与人口加速:补给"需求资源"→ 人口增速大幅提升(+10%/小时,持续消耗) */
const BOOST_GROWTH_PER_H = 0.10;
const BOOST_DURATIONS = [ [1, '1 小时'], [4, '4 小时'], [24, '24 小时'], [-1, '永久'] ];

/* 类地化改造:仅「标准带」星球可改造(严酷星球无法改造),巨量资源 + 后期门槛 */
const TERRAFORM_COST = { metal:5e8, chem:2e8, ice:3e8, deut:1e8 };
const TERRAFORM_REQ  = { lv:5, civ:18 };
...
function techCost(id, lv){ return Math.round(TRAIN_TECHS[id].base * Math.pow(2.2, lv - 1)); }

/* ── 升级耗时(EVE 式先快后慢)+ 影响力加速 ──
   科技/武器/引擎升级支付资源后进入工期,可用影响力或追加科研值提前完工 */
function techTime(lv){   return Math.round(60 * Math.pow(2.6, lv - 1)); }   // 1分→2.6分→…→LV5 46分→LV7 5.1时→LV10 3.8天
function weaponTime(lv){ return Math.round(45 * Math.pow(4,   lv - 1)); }   // 45秒→3分→12分→48分→3.2时
function engineTime(lv){ return Math.round(60 * Math.pow(2.5, lv - 2)); }   // LV2 1分 → LV10 ≈ 25时
...
const BUILDINGS = {
  ...
  academy:    { name:'乘员训练营',   district:'habitation', time:1200,
    cond:{lv:3, civ:8},      fx:{crew:1},    desc:'培养 1 组列车乘员(每组可维护 2 节车厢)' },
  /* 工业区 */
...
const PORT_STORY = [
  { title:'钢铁码头', eyebrow:'航运公报 01 / 03 · 规划院',
    body:[ '随着殖民地扩张,行星之间的物资往来已经超出列车一己之力。规划院呈上一份蓝图:在行星轨道上建造「星港」——不占用地表区划的独立空港,作为自动航运的锚点。',
      '第一座星港的选址毫无悬念:沧澜。问题只剩一个——预算从哪里来。' ],
    choices:[
      { label:'倾斜工业预算,优先开工', sub:'稀有金属 +2000 · 化合物 +600(星港建材)', fx:{ res:{ metal:2000, chem:600 } },
        out:'矿区的三班倒持续了两周。当第一根桁架在沧澜同步轨道上展开时,地面上有人拍下了照片——后来它印在了第一版航运纪念邮票上。' },
      { label:'发行殖民地公债,全民集资', sub:'影响力 +60', fx:{ inf:60 },
        out:'认购窗口排起了长队。一位老工程师买了最大额度:"晨昏号载我们来的时候,我就想过——总有一天,天上会有码头。"' } ] },
  { title:'第一座星港', eyebrow:'航运公报 02 / 03 · 竣工',
    body:[ '星港主体竣工。环形泊位、货运电梯、无人引导阵列——从地面望去,它像一枚静静悬在天顶的银色齿轮。',
      '调度中心发来第一条申请:开通自动贸易线。' ],
    choices:[
      { label:'举行剪彩仪式,全系直播', sub:'影响力 +80', fx:{ inf:80 },
        out:'剪彩那天,沧澜的孩子们第一次在白天看见了"会发光的星星"。教育署顺势把那一天定为航天日。' },
      { label:'跳过仪式,当天投入运营', sub:'科研值 +400 · 影响力 +30', fx:{ rp:400, inf:30 },
        out:'第一艘无人货船在竣工六小时后离港。调度日志的第一行写着:效率即纪念。' } ] },
  { title:'贸易动脉', eyebrow:'航运公报 03 / 03 · 开线',
    body:[ '第一条贸易线开通了。菱形的无人货船沿着固定航线往返,实心满载而去,空心轻盈而归。',
      '从此,星系内的物资将昼夜不停地流动——即使列车远在银河另一端。这张网,会替你看家。' ],
    choices:[
      { label:'将航线网命名为「晨昏动脉」', sub:'影响力 +100 · 文明指数 +0.3', fx:{ inf:100, civ:0.3 },
        out:'名字投票时,"晨昏动脉"以压倒性优势胜出。理由很简单:列车开拓的路,货船替它守着。——《钢铁码头》完' } ] },
];
...
const TUT_SCALE = { hpS:0.9, atkS:0.7, lootS:0.5 };   // 新手战强度(温和)
